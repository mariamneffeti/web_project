...
const baseURL = scriptPath.substring(0, scriptPath.indexOf('/web_project/') + '/web_project/'.length);
      ...
      fetch(`${baseURL}api/add_transaction.php`, {
        method: "POST",
        body: formData,
      })
    ...
    function applyFilters() {
      ...
      const valStart = filterDateStart.value
        ? new Date(filterDateStart.value)
        : null;
      ...
      rows.forEach((row) => {
        ...
        const matchDate =
          (!valStart || rowDate >= valStart) && (!valEnd || rowDate <= valEnd);
