...
async function loadMeetings() {
  try {
    const res  = await fetch("management_api.php?action=get_meetings");
...
async function loadEmployees() {
  try {
    const res   = await fetch("management_api.php?action=get_employees");
...
async function saveMeeting() {
  ...
  const status_el = document.getElementById("addMeetStatus");

  if (!title || !date || !time) {
    status_el.innerHTML = `<div class="alert alert-warning py-2">Please fill in title, date, and time.</div>`;
    return;
  }

  const empIds = Array.from(document.querySelectorAll(".emp-check:checked")).map(c => c.value);

  status_el.innerHTML = `<div class="alert alert-info py-2">Saving...</div>`;

  try {
    const res  = await fetch("management_api.php?action=add_meeting", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title, meeting_date: date, meeting_time: time, meet_link: link, notes, status, employee_ids: empIds })
    });
    const data = await res.json();
    if (data.success) {
      status_el.innerHTML = `<div class="alert alert-success py-2">Meeting added! Notifications sent.</div>`;
      setTimeout(() => {
        bootstrap.Modal.getInstance(document.getElementById("addMeetingModal")).hide();
        status_el.innerHTML = "";
        ["newTitle","newDate","newTime","newLink","newNotes"].forEach(id => document.getElementById(id).value = "");
      ...
      status_el.innerHTML = `<div class="alert alert-danger py-2">${escHtml(data.error || "Error saving meeting.")}</div>`;
    }
  } catch (e) {
    status_el.innerHTML = `<div class="alert alert-danger py-2">Network error.</div>`;
  }
}

// rescheduling (editing) a meeting
...
async function updateMeeting() {
  const id     = document.getElementById("editMeetId").value;
  ...
  if (!date || !time) {
    status_el.innerHTML = `<div class="alert alert-warning py-2">Date and time are required.</div>`;
    return;
  }
  ...
  try {
    const res  = await fetch("management_api.php?action=update_meeting", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, meeting_date: date, meeting_time: time, status })
    });
    const data = await res.json();
    if (data.success) {
...
async function openDetails(id) {
  ...
  try {
    const res  = await fetch(`management_api.php?action=get_meeting_employees&id=${id}`);
...
async function confirmDeleteMeeting() {
  ...
  try {
    const res  = await fetch("management_api.php?action=delete_meeting", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function loadClients() {
  try {
    const res = await fetch("management_api.php?action=get_clients");
...
// simple AI assistant interaction (placeholder)
...
// utility to escape HTML in user-generated content
function escHtml(s) {
  if (s == null) return "";
