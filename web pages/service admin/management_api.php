<?php
  ...
  switch ($action) {
    ...
    case 'get_meeting_employees':
      ...
      if (!$own->fetch()) { echo json_encode([]); break; }
      $stmt = $db->prepare(
    ...
    case 'update_meeting':
      ...
      if (!$meeting) { echo json_encode(['success' => false, 'error' => 'Meeting not found.']); break; }
    ...
    case 'delete_meeting':
      ...
      $check->execute([$id, $company_id]);
      $meeting = $check->fetch(PDO::FETCH_ASSOC);
      if (!$meeting) { echo json_encode(['success' => false, 'error' => 'Not found.']); break; }
  ...
  // Utility function to send emails via Make.com webhook
  function sendEmail(string $to, string $subject, string $message, string $type = 'meeting'): void {
    if (!$to || !filter_var($to, FILTER_VALIDATE_EMAIL)) return;
  
    $MAKE_WEBHOOK = "///"; // replace with your actual Zapier
