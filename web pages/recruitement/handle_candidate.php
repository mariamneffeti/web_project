<?php
    ...
    try{
        ...
        if ($action === "accept"){
            ...
            $stmt = $pdo->prepare("INSERT INTO employees (user_id, company_id, first_name, last_name, hire_date, salary) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$pdo->lastInsertId(), $company_id, $cv['first_name'], $cv['last_name'], date('Y-m-d'), $jobOffer['salary_min']]);
        } 
